...
그림 2.3.1. Hi7-N 제어기 회생저항 과열 에러 관련 부품 배치 
...
그림 2.3.2. Hi7-N 제어기 CNTR에서 저항 값 측정
...
그림 2.4.1. Hi7-N 제어기 CNDR에서 저항 값 측정
...
그림 2.6.1. Hi7-N 제어기 과전압 발생 에러 관련 부품 배치
...
그림 2.9.1. Control 모듈 및 서보구동장치의 교체
...
그림 2.12.1. Hi7-N 제어기 서보 구동장치 출력 케이블 점검
...
그림 2.12.2. Hi7-N 제어기 스위칭 소자 단락 점검
...
그림 2.13.1. Hi7-N 제어기 서보 구동장치 출력 케이블 점검
...
그림 3.2.1. 하드웨어 리밋스위치 설치 위치의 예

![](../_assets/3-Safety-io/E00002/그림2.png)<br>
그림 3.2.2. 하드웨어 리밋스위치 작동 범위의 예
...
우선 티칭펜던트의 전용 입력신호 창에서 리밋(Over-Travel) 항목이 계속 입력되고 있는지를 확인합니다. 이 창은 `창조정` - `선택` - `시스템입력`을 선택하면 볼 수 있습니다. 리밋(Over-Travel) 항목이 황색 표시가 되어 있는면 에러발생를 나타냅니다.<br>
### [주의]
...
그림 4.4.8 로봇의 각 축 모터 위치

(5)	조치 완료후 배선의 통신상태를 점검하십시오.<br>
문제부분의 조치가 완료된 후 아래 그림4.4.9 엔코더 통신실패 모니터링 화면을 참고하여 통신상태를 점검 하십시오.
...
(4) 모터(엔코더)를 교체 시험하십시오.<br>
에러 리셋 후, 주전원 OFF/ON시 계속 에러가 발생하면 모터(엔코더)교체 시험하십시오. 교체한 후 에러가 발생하지 않으면 서보모터의 불량입니다. 서보 모터를 정상품으로 교체하여 주십시오. 아래 그림은 로봇의 각 축 모터의 위치를 나타내고 있으며 다른 로봇은 해당 기구 보수설명서를 참고하여 교체하시기 바랍니다.

![](../_assets/4.서보보드/HS165_motor_position.png)<br>
그림 4.15.4 로봇의 각 축 모터 위치
...
(2)    브레이크 전원 케이블의 단선 또는 접촉 불량 상태를 확인하십시오.<br>
제어기 내부 배선을 점검 하십시오. Hi7-N 제어기의 경우, J12(BD642) 커넥터와 CNOBK(전장보드) 커넥터 간의 배선을 점검하십시오.
